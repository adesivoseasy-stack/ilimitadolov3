/**
 * Content Script - Lovboost - créditos ilimitados
...
// Guardar a fetch originalconst originalFetch = window.fetch;
// Interceptar todas as requisiçõeswindow.fetch = function (...args)  {
  const [resource, config] = args;
  // Se for uma requisição para api.lovable.dev com Authorization header    if (config && config.headers && config.headers['Authorization'])  {
    const authHeader = config.headers['Authorization'];
    if (authHeader.startsWith('Bearer '))  {
      const token = authHeader.replace('Bearer ', '').trim();
      if (token.length > 20)  {
        console.log('[Content] Token capturado da requisição:', token.substring(0, 20) + '...');
        // Salvar no storage da extensão                chrome.storage.local.set( {
          lovable_token: token
        }
        );
      }
  ...
  // Chamar fetch original    return originalFetch.apply(this, args);
};
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) =>  {
  ...
  if (request.action === "getToken")  {
    let token = null;
    try  {
      // Tenta pegar o token do localStorage (múltiplas chaves possíveis)            const possibleKeys = [
                'lovable_token',
                'auth_token',
                'access_token',
                'token',
      ...
      // Se não encontrar em localStorage, procura em sessionStorage            if (!token)  {
        for (const key of possibleKeys)  {
      ...
      // Se não encontrar, procura em qualquer chave que contenha "token" ou "auth"            if (!token)  {
        for (let i = 0;
      ...
      // Salvar no storage da extensão se encontrou            if (token)  {
...
function initialize()  {
  ...
  // Capturar ID do projeto    captureProjectId();
  console.log('[Content] Inicialização concluída');
}
// Iniciar quando o DOM estiver prontoif (document.readyState === 'loading')  {
  document.addEventListener('DOMContentLoaded', initialize);
}  else  {
  initialize();
}
// Também executar imediatamenteinitialize();
